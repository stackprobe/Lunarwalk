...
function @@_SetDate(coName, date, outOfCurrentMonth) { // date: YYYYMMDD
	...
	var selected = Riot_Store.get(Riot_CoNameToParentLoop(coName, 4)).Date == date;
	...
	{
		var routines = Riot_Store.get(Riot_CoNameToParentLoop(coName, 4)).ChangedEvents;
...
function @@_Select(coName) {
	...
	@@^^_SetDate(Riot_CoNameToParentLoop(coName, 4), store.Date);
}
